...
public final class HorizontalPagerWithPageControl extends ViewGroup {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (mTouchState == TOUCH_STATE_HORIZONTAL_SCROLLING) {
                    ...
                    //api for addview when scrollx changed
                    if (mOnFlingListener != null) {
                        mOnFlingListener.onFling(deltaX);
                    }
                    if (deltaX < 0) {
                        if (scrollX > 0) {
     ...
     */
    public void setOnFlingListenet(final OnFlingListener onFlingListener) {
...
//        setActiveInactiveDrawables(whichScreen);
    ...
    /**
     * Listener for the event that the scrollx been changed.
     */
    public static interface OnFlingListener {
        /**
         * Notifies listeners onfling. Runs as soon as the fling start.
         *
         * @param the scrolled x
         */
        void onFling(int Scrollx);
    }
	...
	public void addPagerControl() {
		...
		points = initPageControl();
		view.addView(points);
	}
	public void freshPagerControl(){
		ViewGroup view = (ViewGroup) this.getParent();
		view.removeView(points);
		points = initPageControl();
		view.addView(points);
	}
